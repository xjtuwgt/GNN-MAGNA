...
def HypeParameterSpace(data_name):
    ...
    top_k = {'name': 'top_k', 'type': 'choice', 'values': [3, 5, 7, 9, 10]}
    topk_type = {'name': 'topk_type', 'type': 'fixed', 'value': 'local'}
    epochs = {'name': 'epochs', 'type': 'fixed', 'value': 800}
    ...
    hop_num = {'name': 'hop_num', 'type': 'fixed', 'value': 10}
    neg_slope = {'name': 'negative_slope', 'type': 'range', 'bounds': [0.1, 0.6]}
    adam_weight_decay = {'name': 'adam_weight_decay', 'type': 'range', 'bounds': [1e-5, 1e-6], "log_scale": True}
...
if __name__ == '__main__':
    generate_random_search_bash(data_name='cora', task_num=200)