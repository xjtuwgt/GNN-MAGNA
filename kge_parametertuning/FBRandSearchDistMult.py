...
def HypeParameterSpace(model_name):
    learning_rate = {'name': 'learning_rate', 'type': 'range', 'bounds': [0.0001, 0.005], "log_scale": True}
    smooth_factor = {'name': 'gamma', 'type': 'range', 'bounds': [0.1, 0.6]}
    ...
    # batch_size = {'name': 'batch_size', 'type': 'choice', 'values': [2048, 3076]}
    ...
    max_steps = {'name': 'max_steps', 'type': 'fixed', 'value': 120000}
    layers = {'name': 'layers', 'type': 'fixed', 'value': 2}
    alpha = {'name': 'alpha', 'type': 'range', 'bounds': [0.05, 0.5]}
    # hops = {'name': 'hops', 'type': 'choice', 'values': [2, 3]}
    hops = {'name': 'hops', 'type': 'fixed', 'value': 2}
...
if __name__ == '__main__':
    generate_random_search_bash(model_name='DistMult', data_name='FB15k-237', task_num=60)